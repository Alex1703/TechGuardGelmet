...
class Bot():
    ...
    def send_location(self,chat_id, latitude, longitude,parse_mode='MarkdownV2', reply_markup=None):
              
        try:
            message = urequests.get(self.url+ f'/sendLocation?latitude={latitude}&longitude={longitude}&chat_id={chat_id}"')
            assert message
            message.close()

        except Exception:
            print('Ubicación not sent')
